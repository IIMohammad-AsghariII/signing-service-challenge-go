...
func TestVerifySignatureAfterSigningEcc(t *testing.T) {
	...
	valid, err := verifySignatureEcc(publicKey, signature, signedData)
	if err != nil {
...
func verifySignatureEcc(publicKeyStr, signatureStr, signedDataStr string) (bool, error) {
	// Load the public key
	...
	valid := ecdsa.Verify(ecdsaPublicKey, hashed, new(big.Int).SetBytes(r), new(big.Int).SetBytes(s))
	if !valid {
